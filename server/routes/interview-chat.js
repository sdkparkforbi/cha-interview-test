// 면담봇 LLM 핸들러 (Middleton Express)
// model: 'gpt' (기본, GPT-4o-mini) | 'gemma' (관리자, Gemma4 via Ollama)
...
router.post('/interview-chat', async (req, res) => {
  ...
  try {
    ...
    if (useGemma) {
      // Gemma4 via Ollama OpenAI-compat endpoint (기존 서버 env 재사용)
      const EXAONE_BASE = process.env.EXAONE_BASE_URL || 'http://localhost:11435/v1';
      const GEMMA_MODEL = process.env.EXAONE_MODEL || 'gemma4:latest';
      const EXAONE_KEY = process.env.EXAONE_API_KEY || 'router-key';
      ...
      const response = await fetch(`${EXAONE_BASE}/chat/completions`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${EXAONE_KEY}`,
